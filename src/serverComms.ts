...
import { HEADER_CLOSE, HEADER_CALLBACK_PING, HEADER_INCOMING_IMAGE, MsgClose, MsgCallbackPing, MsgCsv, MsgKey, MsgImage } from "./messages.js"

/**An all-in-one object for server functions and communications.*/
class ServerComms {
    clients: Map<string, WebSocket>
    server: WebSocketServer
    awaitingCallback: Array<string>
    ...
    handleMsgClose(msg: MsgClose) {
        this.clients.delete(msg.clientKey)
    }

    handleMsgCsv(msg: MsgCsv) {
    ...
    handleCallbackPing(msg: MsgCallbackPing) {
    ...
    callbackPing() {
        this.awaitingCallback.forEach((value, _, __) => {
            this.send(this.clients.get(value) as WebSocket, new MsgClose(value))
    ...
    send(socket: WebSocket, data: object) {
